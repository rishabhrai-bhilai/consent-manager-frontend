...
const RequestorConsentHistory = () => {
  ...
  useEffect(() => {
    const fetchConsentHistory = async () => {
      ...
      try {
        setLoading(true);
        console.log("Fetching consent history for requestor userId:", userId);
        const response = await axios.get(`/requestor/${userId}/getRequestorConsentHistoryByUserId`);
  ...
  const formatDetails = (data) => {
    const providerDetails = data.additional_info.split(", ").reduce((acc, pair) => {
      ...
      <div className="flex flex-col sm:flex-row text-sm text-gray-700 gap-6">
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div>
              <span className="font-semibold text-gray-900">Provider Name:</span> {data.provider_name}
            </div>
            <div>
        ...
        <div className="flex-1">
          <h4 className="text-md font-semibold text-gray-900 mb-2">Provider Details</h4>
          <div className="grid grid-cols-1 gap-4">
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-gray-600">
                        {item.provider_name} • {formatDate(item.requested_at)}
                      </p>
                    </div>
...
export default RequestorConsentHistory;